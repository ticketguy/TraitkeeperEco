...
// Browser and device detection for wallet compatibility
...
// Show browser-specific wallet installation guidance
function showWalletGuidance() {
    const browser = detectBrowserAndDevice();
    let message = '';
    let recommendedWallets = [];

    if (browser.isMobile) {
        if (browser.isIOS) {
            message = 'On iOS, you can use mobile wallet apps like Phantom or Solflare.';
            recommendedWallets = [
                { name: 'Phantom', url: 'https://phantom.app/download', appStore: 'https://apps.apple.com/app/phantom-solana-wallet/id1598432977' },
                { name: 'Solflare', url: 'https://solflare.com/download', appStore: 'https://apps.apple.com/app/solflare/id1580902717' }
            ];
        } else if (browser.isAndroid) {
            message = 'On Android, you can use mobile wallet apps like Phantom or Solflare.';
            recommendedWallets = [
                { name: 'Phantom', url: 'https://phantom.app/download', playStore: 'https://play.google.com/store/apps/details?id=app.phantom' },
                { name: 'Solflare', url: 'https://solflare.com/download', playStore: 'https://play.google.com/store/apps/details?id=com.solflare.mobile' }
            ];
        }
    } else {
        // Desktop browser
        if (browser.isChrome || browser.isBrave || browser.isEdge) {
            message = 'Install a Solana wallet extension from the Chrome Web Store.';
            recommendedWallets = [
                { name: 'Phantom', url: 'https://chrome.google.com/webstore/detail/phantom/bfnaelmomeimhlpmgjnjophhpkkoljpa' },
                { name: 'Solflare', url: 'https://chrome.google.com/webstore/detail/solflare-wallet/bhhhlbepdkbapadjdnnojkbgioiodbic' },
                { name: 'Backpack', url: 'https://chrome.google.com/webstore/detail/backpack/aflkmfhebedbjioipglgcbcmnbpgliof' }
            ];
        } else if (browser.isFirefox) {
            message = 'Install a Solana wallet extension from Firefox Add-ons.';
            recommendedWallets = [
                { name: 'Phantom', url: 'https://addons.mozilla.org/firefox/addon/phantom-app/' },
                { name: 'Solflare', url: 'https://addons.mozilla.org/firefox/addon/solflare-wallet/' }
            ];
        } else if (browser.isSafari) {
            message = 'Safari has limited wallet support. We recommend using Chrome, Firefox, or Brave for the best experience.';
            recommendedWallets = [
    ...
    return { message, recommendedWallets, browser };
}
