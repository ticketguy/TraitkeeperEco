...
class IndexerService:
    """
    High-level orchestrator with 90-99% efficiency.
    ...
    def __init__(self):
        ...
        logger.info("✅ IndexerService initialized with 90%+ efficiency features")
    ...
    # ==================== MARKET STATS & BLOCKCHAIN VOLUME ====================

    async def fetch_and_store_all_market_stats(self, collection):
        """
        Fetches raw market stats in parallel from all active API providers
    ...
    async def calculate_and_store_blockchain_volume(self, collection):
        ...
        try:
            ...
            logger.error(f"Error calculating blockchain volume for {collection.name}: {e}", exc_info=True)
